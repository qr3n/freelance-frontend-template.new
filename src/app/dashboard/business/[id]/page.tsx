...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string, setup: string }>, searchParams: Promise<{ setup?: string }> }) {
  ...
  return (
    <div className={`bg-white w-full h-[100dvh]`}>
      { setup && <SetupTablesModal businessId={id}/> }
      ...
      <Lenis className="h-[calc(100dvh)] overflow-y-auto relative">
        <div className=" relative flex flex-col items-center pb-24">
          <DishesList businessId={id} initialDishes={initialDishes} />
        </div>
      </Lenis>
    </div>
  );
